...
export default function Home() {
  ...
  return (
    <TonConnectUIProvider manifestUrl="https://ton-connect.github.io/demo-dapp-with-react-ui/tonconnect-manifest.json">
      {user && <WalletAutoSaver userId={user.id} onWalletSaved={(addr) => setUserProfile({...userProfile, wallet_address: addr})} />}
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden relative pb-20 lg:pb-0">
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden">
                  ...
                  <div className="p-8 space-y-8">
                        ...
                        <div className="flex-1">
                          ...
                          {isEditingEmail ? (
                            <div className="flex gap-2 mt-2">
                              <input type="email" placeholder="vash_email@gmail.com" className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold outline-none w-full max-w-[250px]" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} />
                              <button onClick={handleLinkEmail} className="bg-green-500 text-white p-2 rounded-xl hover:bg-green-600 transition-colors"><Check size={18} /></button>
                              <button onClick={() => setIsEditingEmail(false)} className="bg-gray-200 text-gray-500 p-2 rounded-xl hover:bg-gray-300 transition-colors"><X size={18} /></button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-bold text-gray-800 break-all">{user.email}</p>
                              {user.email?.startsWith('tg_') && <span className="bg-yellow-100 text-yellow-700 text-[9px] font-black px-2 py-0.5 rounded uppercase">Тимчасова</span>}
                            </div>
                          )}
                        </div>
                        {!isEditingEmail && user.email?.startsWith('tg_') && (
                          <button onClick={() => setIsEditingEmail(true)} className="px-5 py-2.5 bg-blue-50 text-[9px] font-black uppercase rounded-xl text-blue-600 hover:bg-blue-100 transition-colors flex items-center gap-2 whitespace-nowrap">
                      ...
                      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 pb-8 border-b border-gray-50">
                        <div className="flex-1">
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Пароль для сайту</p>
                          <div className="flex gap-2 mt-2">
                             <input type="password" placeholder="Новий пароль (мін. 6 симв.)" className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold outline-none w-full max-w-[250px]" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
                             <button onClick={handleSetPassword} className="bg-purple-600 text-white px-4 py-2 rounded-xl font-bold text-xs uppercase hover:bg-purple-700 transition-colors">Зберегти</button>
                          </div>
                      ...
                      {userProfile?.telegram_id ? (
                        <div className="p-6 bg-green-50 rounded-[2rem] border border-green-100 shadow-sm mb-6 relative overflow-hidden">
                          <div className="absolute top-0 right-0 p-4 opacity-10 text-green-600"><ShieldCheck size={80} /></div>
                          <div className="flex items-center gap-3 mb-4 relative z-10">
                            <div className="p-2 bg-green-500 text-white rounded-xl shadow-lg shadow-green-200"><Check size={18} strokeWidth={3} /></div>
                            <div>
                          ...
                          <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl border border-green-100/50 relative z-10">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="overflow-hidden">
                                <p className="text-xs font-black text-gray-800 truncate">{userProfile.full_name || 'Користувач Telegram'}</p>
                                <p className="text-[9px] font-bold text-gray-400 font-mono mt-0.5">ID: {userProfile.telegram_id}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="p-6 bg-blue-50 rounded-[2rem] border border-blue-100 shadow-inner mb-6">
                          ...
                          <button onClick={() => setIsMergeModalOpen(true)} className="w-full py-3 bg-white text-blue-600 rounded-xl text-[10px] font-black uppercase border border-blue-200 shadow-sm active:scale-95 transition-all">
                      ...
                      {/* --- КРИПТО ГАМАНЕЦЬ --- */}
                      <div className="p-6 bg-gray-900 rounded-[2rem] shadow-lg mb-6 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-20 text-white"><Wallet size={80} /></div>
                        <div className="flex items-center gap-3 mb-4 relative z-10">
                          <div className="p-2 bg-white/10 text-white rounded-xl backdrop-blur-md"><Wallet size={18} /></div>
                          <p className="text-[10px] font-black text-white uppercase tracking-widest">Крипто Гаманець</p>
                        </div>
                        <div className="relative z-10">
                           <TonConnectButton className="w-full" />
                           {userProfile?.wallet_address && (
                             <p className="mt-3 text-[9px] font-bold text-gray-400 font-mono bg-black/30 p-2 rounded-lg text-center break-all border border-white/10">
                               {userProfile.wallet_address}
                             </p>
                           )}
                        </div>
                      ...
                      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 pb-8 border-b border-gray-50">
                        ...
                        {userProfile?.subscription_tier !== 'pro' && (
                          <button onClick={handleBuyPro} disabled={isLoading} className="px-5 py-2.5 bg-purple-600 text-white text-[9px] font-black uppercase rounded-xl hover:bg-purple-700 transition-all shadow-lg shadow-purple-200 flex items-center gap-2">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {profiles.map((p: any) => (
                        <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="p-6">
                            <button onClick={() => toggleSubscription(p.id, p.subscription_tier)} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${p.subscription_tier === 'pro' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-xl border-t border-gray-100 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
            <div className="flex items-center justify-between w-full max-w-md mx-auto px-10 py-3">
              <button onClick={() => { setActiveTab('feed'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'feed' ? 'text-purple-600' : 'text-gray-400'}`}>
              ...
              {canPost && (
                <button onClick={() => setIsModalOpen(true)} className="w-14 h-14 bg-gray-900 text-white rounded-2xl flex items-center justify-center shadow-lg active:scale-95 transition-transform -translate-y-4 border-4 border-white">
                  <Plus size={28} />
                </button>
              )}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center lg:p-4 bg-black/95 backdrop-blur-md" onClick={() => setSelectedAd(null)}>
            ...
            <div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} className="relative w-full lg:max-w-6xl h-full lg:h-[90vh] bg-white lg:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row animate-in slide-in-from-bottom duration-300" onClick={e => e.stopPropagation()}>
              ...
              <div className="w-full lg:w-1/2 h-[45vh] lg:h-full bg-gray-950 flex items-center justify-center relative border-b lg:border-b-0 lg:border-r border-gray-100 group select-none">
                {(() => {
                  const mediaUrls = selectedAd.image?.includes(',') ? selectedAd.image.split(',').map((url: string) => url.trim()).filter(Boolean) : (selectedAd.image ? [selectedAd.image] : []);
        ...
        {isMergeModalOpen && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200" onClick={() => setIsMergeModalOpen(false)}>
            <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
              <button onClick={() => setIsMergeModalOpen(false)} className="absolute top-6 right-6 text-gray-400 hover:text-gray-600 transition-colors"><X size={24} /></button>
              <div className="text-center mb-8">
                <div className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-200"><Globe className="text-white" size={28} /></div>
              ...
              <div className="space-y-4">
                <input type="email" placeholder="Ваш Email" className="w-full h-14 bg-gray-50 border border-gray-100 rounded-2xl px-5 text-sm font-bold outline-none focus:border-blue-600 transition-all" value={mergeEmail} onChange={e => setMergeEmail(e.target.value)} />
                <input type="password" placeholder="Ваш Пароль" className="w-full h-14 bg-gray-50 border border-gray-100 rounded-2xl px-5 text-sm font-bold outline-none focus:border-blue-600 transition-all" value={mergePassword} onChange={e => setMergePassword(e.target.value)} />
                <button onClick={handleMergeAccount} disabled={isLoading} className="w-full h-14 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all mt-4">{isLoading ? 'З\'єднуємо...' : 'Підтвердити зв\'язок'}</button>
              </div>
            </div>
          </div>
        )}
