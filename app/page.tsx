...
import { 
  ...
  Smartphone, MessageCircle, Mic, Share2
} from 'lucide-react';
...
export default function Home() {
  ...
  const formatsList = [
    { id: 'Image', label: 'Зображення', icon: <Smartphone size={14}/> },
    { id: 'Video', label: 'Відео', icon: <Play size={14}/> },
    { id: 'Circle', label: 'Кружечок (Circle)', icon: <MessageCircle size={14}/> },
    { id: 'Audio', label: 'Голосове (Audio)', icon: <Mic size={14}/> },
    { id: 'Forward', label: 'Переслане (Forward)', icon: <Share2 size={14}/> }
  ];
  ...
  // Регулярний вираз для справжніх емодзі
  ...
  const saveNewAd = async () => {
    ...
    try {
      ...
      setNewAd({ title: '', mainText: '', format: 'Image', categories: ['Інше'], language: 'Українська', hasEmoji: false, buttons: ['Дізнатися більше'], image: null, file: null, type: 'text' });
  ...
  const filteredAds = ads.filter((ad) => {
    ...
    return matchesSearch && matchesCategory && matchesFormat && matchesLanguage && matchesEmoji && matchesButtons;
  });
    ...
    <div className="min-h-screen bg-[#f0f2f5] flex font-sans text-gray-900">
      ...
      <aside className="w-80 bg-white border-r border-gray-200 hidden lg:flex flex-col sticky h-screen top-0">
        <div className="p-6 border-b border-gray-100 flex-shrink-0 flex items-center gap-3">
          ...
          <span className="font-black text-lg text-purple-600 uppercase italic leading-none tracking-tighter">Absolute Spy</span>
        </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <p className="text-[9px] font-bold text-gray-400 px-1 uppercase">Категорія</p>
                <select value={filters.category} onChange={(e) => setFilters({...filters, category: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold outline-none cursor-pointer">
                  <option value="Всі">Всі ніші</option>
                  {categoriesList.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <p className="text-[9px] font-bold text-gray-400 px-1 uppercase">Формат контенту</p>
                <select value={filters.format} onChange={(e) => setFilters({...filters, format: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold outline-none cursor-pointer">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white p-6 border-b border-gray-100 shadow-sm z-10">
          <div className="max-w-4xl mx-auto relative group">
            ...
            <input type="text" placeholder="Швидкий пошук за креативами..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full h-14 bg-gray-100 rounded-2xl pl-14 pr-6 font-bold text-gray-700 outline-none focus:bg-white focus:ring-4 focus:ring-purple-600/5 transition-all" />
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8f9fc] no-scrollbar">
          <div className="max-w-5xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-10">
              {filteredAds.map((ad) => (
                <div key={ad.id} onClick={() => setSelectedAd(ad)} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group cursor-pointer relative flex flex-col h-[450px]">
                  <button onClick={(e) => {e.stopPropagation(); deleteAd(ad.id, e);}} className="absolute top-4 right-4 z-20 p-2 bg-white/80 backdrop-blur rounded-full text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                    <Trash2 size={16} />
                  </button>
                  <div className="h-48 bg-gray-100 relative overflow-hidden flex items-center justify-center">
                    {ad.image ? (
                      ad.type === 'video' ? <video src={ad.image} className="w-full h-full object-cover" muted /> : <img src={ad.image} className="w-full h-full object-cover" alt="" />
                    ) : <FileText className="text-purple-100" size={40} />}
                    <div className="absolute top-4 left-4 flex flex-wrap gap-1 max-w-[80%] z-10">
                      {Array.isArray(ad.category) ? (
                        ad.category.map((cat, i) => (
                          <div key={i} className="bg-purple-600/90 backdrop-blur-md px-2 py-1 rounded-full flex items-center gap-1 shadow-sm">
                            ...
                            <span className="text-[8px] font-black text-white uppercase">{cat}</span>
                          </div>
                        ))
                      ) : ad.category && (
                          <div className="bg-purple-600/90 backdrop-blur-md px-2 py-1 rounded-full flex items-center gap-1 shadow-sm">
                            <Tag size={8} className="text-white" />
                            <span className="text-[8px] font-black text-white uppercase">{ad.category}</span>
                          </div>
                      )}
                    </div>
                  </div>
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="text-[9px] font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-md">{ad.language}</div>
                    </div>
                    <h3 className="font-bold text-gray-800 text-sm mb-4 line-clamp-3 leading-snug flex-1">{ad.title}</h3>
                    <div className="flex gap-2 pt-4 border-t border-gray-50 items-center">
                       {(ad.has_emoji === true || (ad.mainText && emojiRegex.test(ad.mainText))) && <span className="text-xs" title="Містить емодзі">😃</span>}
                       {ad.has_buttons && <span className="text-xs" title="Має кнопки">🖱️</span>}
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="space-y-8 flex-1">
                <div className="relative group">
                  <div className="flex justify-between items-center mb-3">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><AlignLeft size={14}/> Текст оголошення</p>
                    <div className="flex gap-2">
                      <button onClick={() => copyToClipboard(selectedAd.mainText)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-50 text-purple-600 text-[10px] font-bold hover:bg-purple-600 hover:text-white transition-all">
                        {copied ? <Check size={12}/> : <Copy size={12}/>} {copied ? 'Є!' : 'Текст'}
                      </button>
                    </div>
                  </div>
                  <div className="p-7 bg-gray-50/50 rounded-[2rem] border border-gray-100 shadow-inner max-h-[300px] overflow-y-auto no-scrollbar">
                    <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap break-words font-medium italic italic">
      ...
      {/* --- МОДАЛЬНЕ ВІКНО: ДОДАТИ КРЕАТИВ (ПОВНЕ) --- */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200 max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-black text-purple-600 uppercase tracking-tighter italic">Новий Креатив</h2>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select value={newAd.format} onChange={(e) => setNewAd({...newAd, format: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold outline-none">
                    {formatsList.map(f => <option key={f.id} value={f.id}>{f.label}</option>)}
                  </select>
                </div>
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase text-gray-400 px-1">Мова</p>
                  <select value={newAd.language} onChange={(e) => setNewAd({...newAd, language: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold outline-none">
              ...
              {/* Чекбокс Емодзі */}
              <label className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl cursor-pointer border border-transparent hover:border-purple-200 transition-all">
                <input type="checkbox" checked={newAd.hasEmoji} onChange={(e) => setNewAd({...newAd, hasEmoji: e.target.checked})} className="w-5 h-5 rounded accent-purple-600" />
                <span className="text-xs font-black uppercase text-gray-500 tracking-wider">Містить емодзі 😃</span>
              </label>
              ...
              {/* Медіа */}
              <div className="relative h-36 bg-purple-50 rounded-3xl border-2 border-dashed border-purple-200 flex flex-col items-center justify-center overflow-hidden hover:border-purple-400 transition-colors group cursor-pointer">
                {newAd.image ? (
                  ...
                  <> <Upload className="text-purple-300 group-hover:text-purple-500 mb-1" size={24} /> <span className="text-[9px] font-black text-purple-400 uppercase tracking-widest">Завантажити медіа</span> </>
                )}
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  <p className="text-[10px] font-black uppercase text-gray-400">Категорії ({newAd.categories.length}/3)</p>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {newAd.categories.map((cat, idx) => (
                    <div key={idx} className="flex gap-2">
                      ...
                      {newAd.categories.length > 1 && <button onClick={() => removeCategoryField(idx)} className="text-gray-300 hover:text-red-500 transition-colors"><X size={16}/></button>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                 <p className="text-[10px] font-black uppercase text-purple-600 tracking-widest">Кнопки ({newAd.buttons.filter(b => b.trim() !== '').length})</p>
                 <div className="grid grid-cols-2 gap-2">
                    {newAd.buttons.map((btn, idx) => (
                      <input key={idx} type="text" value={btn} onChange={(e) => {
                        const nb = [...newAd.buttons]; nb[idx] = e.target.value; setNewAd({...newAd, buttons: nb});
                      }} className="bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-[10px] font-bold outline-none focus:bg-white transition-all" placeholder="Текст кнопки" />
                    ))}
                    {newAd.buttons.length < 10 && <button onClick={() => setNewAd({...newAd, buttons: [...newAd.buttons, '']})} className="text-[9px] font-bold text-purple-600 bg-purple-50 rounded-xl py-2 hover:bg-purple-100 transition-colors">+ Додати</button>}
                 </div>
              </div>
            </div>

            <div className="p-8 bg-gray-50/50">
              <button onClick={saveNewAd} disabled={isLoading} className="w-full py-5 bg-purple-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl shadow-purple-600/20 hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-50">
                {isLoading ? 'Збереження...' : 'Опублікувати креатив'}
              </button>
            </div>
          </div>
        </div>
